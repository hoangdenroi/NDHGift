    ...
    <div
        ...
        <div class="p-8 sm:p-10">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-5" x-data="{
                    ...
                    get hasMaxLength() { return this.password.length <= 32 && this.password.length > 0; },
                    ...
                    get isConfirmed() { return this.password === this.password_confirmation && this.password_confirmation.length > 0; },
                    get isPasswordSecure() { return this.hasMinLength && this.password.length <= 32 && this.hasLowercase && this.hasUppercase && this.hasNumber && this.hasSpecial; },
                    get isValid() { return this.isPasswordSecure && this.isConfirmed; }
                }"
                @submit="if (isTouched && !isValid) { $event.preventDefault(); window.dispatchEvent(new CustomEvent('toast', { detail: { type: 'error', title: 'Lỗi', message: 'Mật khẩu chưa đáp ứng đúng tiêu chuẩn bảo mật.' } })); }">
                @csrf

                {{-- Họ tên --}}
                <div class="space-y-1.5">
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300" for="name">Họ
                        tên</label>
                    <div class="relative">
                        <span
                ...
                <div class="space-y-1.5">
                    ...
                    <div x-show="isTouched && !isPasswordSecure" x-cloak
                        class="mt-2 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800/50 space-y-2 text-[12px] transition-all duration-300">
                        <p class="font-bold text-slate-600 dark:text-slate-400 mb-1 flex items-center gap-1.5">
                ...
                <div class="space-y-1.5">
                    ...
                    <div x-show="isConfirmTouched && password_confirmation.length > 0" x-cloak
                        ...
                        <div class="flex items-center gap-1.5 text-[12px]"
                            ...
                            <span x-text="isConfirmed ? 'Mật khẩu trùng khớp' : 'Mật khẩu xác nhận không khớp'"></span>
                        </div>
                    </div>
                </div>

                {{-- Nút đăng ký --}}
                <button type="submit" :disabled="isTouched && !isValid"
                    :class="isTouched && !isValid ? 'opacity-60 cursor-not-allowed bg-slate-400 dark:bg-slate-700 text-slate-200 shadow-none' : 'bg-primary hover:bg-blue-700 text-white shadow-blue-500/20 active:scale-[0.98]'"
                    class="w-full flex items-center justify-center h-12 font-bold rounded-xl transition-all shadow-md">
                    Đăng Ký
                </button>
            </form>
