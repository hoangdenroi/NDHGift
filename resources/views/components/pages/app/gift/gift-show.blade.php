    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div class="space-y-6">
            <div class="bg-app-surface border border-app-border rounded-3xl overflow-hidden shadow-lg group relative">
                ...
                @if($giftTemplate->is_hot)
                    <span class="absolute top-4 left-4 z-10 px-3 py-1 rounded-xl bg-red-600 text-xs font-black text-white uppercase tracking-wider select-none flex items-center gap-1 shadow-md">
            ...
            <!-- Nút quay lại danh sách ở góc dưới -->
            <div class="flex items-center justify-start">
                <a href="{{ route('app.gift.index', ['locale' => app()->getLocale()]) }}"
                    class="inline-flex items-center gap-1.5 py-2 px-4 rounded-xl border border-app-border bg-app-surface hover:bg-app-main/5 text-xs font-bold text-app-text transition-all active:scale-[0.98]">
                    <span class="material-symbols-outlined text-[16px]">arrow_back</span>
                    <span>{{ __('Quay lại danh sách') }}</span>
                </a>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-app-surface border border-app-border rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-app-border/40 pb-4">
                    ...
                    <div class="flex items-center gap-4 text-xs text-app-muted">
                        ...
                        <div class="flex items-center gap-0.5">
                            @for($i = 1; $i <= 5; $i++)
                                <span class="material-symbols-outlined text-sm text-amber-500 fill-current select-none">star</span>
                            @endfor
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <span>{{ __('Đã bán:') }} <strong class="text-app-text">{{ number_format($giftTemplate->sold) }}</strong></span>
                        </div>
                    </div>
                </div>
                ...
                {{-- Tên & Mô tả --}}
                <div class="space-y-3">
                    <h2 class="text-2xl sm:text-3xl font-black text-app-text tracking-tight">{{ $giftTemplate->name }}</h2>
                    <p class="text-sm text-app-muted leading-relaxed text-justify whitespace-pre-line">{{ $giftTemplate->description }}</p>
                </div>

                {{-- Khung Giá bán --}}
                <div class="bg-app-main/5 border border-app-border/50 rounded-2xl p-4 flex items-center justify-between">
                    ...
                    @if($giftTemplate->discount > 0)
                        <span class="px-3 py-1.5 rounded-xl bg-rose-500 text-white text-xs font-black select-none shadow-sm shadow-rose-500/10">
                ...
                <div class="space-y-3.5">
                    <h4 class="text-xs font-bold text-app-text uppercase tracking-wider">{{ __('Đặc điểm nổi bật:') }}</h4>
