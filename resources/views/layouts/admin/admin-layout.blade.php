...
<head>
    ...
    <title>{{ $title ?? 'Admin - NDHGift' }}</title>
...
<body x-data="{ sidebarOpen: false, sidebarMini: false, isPanelOpen: false }"
                ...
                const showToast = () => {
                    window.dispatchEvent(new CustomEvent('toast', {
                        detail: {
                            ...
                            message: @js(session('toast_message')),
                            link: @js(session('toast_link')),
                            linkText: @js(session('toast_link_text')),
                            output: @js(session('sync_output'))
                        }
                    }));
                };
    ...
    {{-- Dispatch toast cũ (success/error) để tương thích ngược --}}
