    ...
    <div x-show="open" @click.outside="open = false" @keydown.escape.window="open = false"
        ...
        <div x-show="showExchange" 
            ...
            <div class="text-center pb-1">
                <span class="text-xs text-app-text font-bold block mb-0.5">{{ __('Currency Converter') }}</span>
                <span class="text-[10px] text-primary font-semibold flex items-center justify-center gap-1">
                    <span>{{ __('Exchange Rate') }}: 1 USD ($) ≈</span>
                    <span x-text="loadingRate ? '...' : new Intl.NumberFormat('vi-VN', { maximumFractionDigits: 2 }).format(rate)"></span>
                    <span>VND</span>
                </span>
            </div>
            ...
            <div class="flex flex-col gap-2.5">
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-bold text-app-muted" x-text="isVndToUsd ? '{{ __('VND Amount') }} ({{ __('Input') }})' : '{{ __('USD Amount') }} ({{ __('Input') }})'"></label>
                    <div class="relative">
                        {{-- Ô nhập VND --}}
                        <template x-if="isVndToUsd">
                            <input type="text" :value="vndAmountFormatted" @input="handleVndInput($event.target.value)"
                                ...
                                :placeholder="'{{ __('Enter VND amount') }}'" />
                        </template>
                        {{-- Ô nhập USD --}}
                        <template x-if="!isVndToUsd">
                            <input type="text" :value="usdAmountFormatted" @input="handleUsdInput($event.target.value)"
                                class="w-full h-9 px-3 pr-12 rounded-lg border border-app-border bg-app-surface text-app-text text-xs focus:border-primary focus:ring-primary outline-none"
                                :placeholder="'{{ __('Enter USD amount') }}'" />
                        </template>
                        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-bold text-app-muted" x-text="isVndToUsd ? 'VND' : 'USD ($)'"></span>
                    </div>
                </div>
                ...
                <div class="flex justify-center -my-1">
                    <button @click="isVndToUsd = !isVndToUsd; clearInputs();" type="button"
                        ...
                        <span class="material-symbols-outlined text-[18px]" :class="loadingRate ? 'animate-spin' : ''">swap_vert</span>
                    </button>
                </div>

                {{-- TRƯỜNG KẾT QUẢ PHÍA DƯỚI (READONLY/DISPLAY OUTPUT) --}}
                <div class="flex flex-col gap-1">
                    <label class="text-[9px] font-bold text-app-muted" x-text="isVndToUsd ? '{{ __('USD Amount') }} ({{ __('Result') }})' : '{{ __('VND Amount') }} ({{ __('Result') }})'"></label>
                    <div class="relative">
                        {{-- Kết quả USD --}}
                        <template x-if="isVndToUsd">
                        ...
                        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-bold text-app-muted" x-text="isVndToUsd ? 'USD ($)' : 'VND'"></span>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-app-border pt-2.5">
                ...
                <div class="flex justify-between items-center text-[10px] text-app-muted">
                    <span>{{ __('Convert Wallet Balance:') }}</span>
